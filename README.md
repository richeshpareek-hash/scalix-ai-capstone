...
- The Dashboard uses Overall Capacity Position as its single headline capacity metric; the separate Overall ACRS card is retained in Business Analytics rather than duplicated in the executive KPI row.
- The executive Dashboard removes redundant six-month RAG count tiles, projected-equity-load tiles and low-value risk-theme count metadata; expanded Decision Agenda evidence uses high-contrast white text.
