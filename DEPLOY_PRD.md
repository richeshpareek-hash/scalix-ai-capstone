...
- **1:30–2:30 — Live solution demo:** open the public link, sign in as ClearOne, show deterministic baseline ACRS and the optional BYOK Analysis toggle, run the 4M-order Ask Scalix scenario, review baseline-versus-scenario service changes, and show that recommendations await approval. Then run the unauthorized-production-action case and show refusal/escalation. State that no key ships and that production would move model access behind a backend.
